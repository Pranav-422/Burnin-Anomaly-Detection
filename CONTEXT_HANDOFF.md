...
- `run_pipeline.py` — full orchestration, runs clean end-to-end. Verified: 100% recall (0 false negatives) both Module A and combined; calibrated precision: Module A ~85%, Module B ~91%, Combined ~80% (calibrated with SPC thresholds: IQR k=2.0, IF contamination=0.02, safety slope=99.5th percentile); per-parameter test-set MAE: leakage ~0.36µA, Iddq ~0.64µA, prop delay ~0.13ns.
